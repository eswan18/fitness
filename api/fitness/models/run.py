...
class Run(BaseModel):
    ...
    def from_mmf(cls, mmf_run: MmfActivity) -> Self:
        ...
        # Generate deterministic ID for MMF runs
        # Use a combination of date, distance, duration, and source
        id_string = f"mmf_{workout_date.isoformat()}_{mmf_run.distance}_{mmf_run.workout_time}"
        # Create a shorter hash to keep IDs manageable
        id_hash = hashlib.sha256(id_string.encode()).hexdigest()[:12]
        run_id = f"mmf_{id_hash}"
    ...
    def from_strava(cls, strava_run: StravaActivityWithGear) -> Self:
        # Use Strava's existing ID with a prefix
        run_id = f"strava_{strava_run.id}"
