...
class SyncedRun(BaseModel):
    ...
    id: int
    run_id: str
    run_version: int = Field(default=1, description="Version of the run that was synced")
...
class SyncResponse(BaseModel):
    ...
    google_event_id: Optional[str] = Field(default=None, description="Google Calendar event ID if successful")
    sync_status: SyncStatus = Field(description="Current sync status")
    synced_at: Optional[datetime] = Field(default=None, description="When sync occurred if successful")
...
class SyncStatusResponse(BaseModel):
    ...
    sync_status: Optional[SyncStatus] = Field(default=None, description="Sync status if synced")
    synced_at: Optional[datetime] = Field(default=None, description="When sync occurred if synced")
    google_event_id: Optional[str] = Field(default=None, description="Google Calendar event ID if synced")
    run_version: Optional[int] = Field(default=None, description="Version that was synced")
    error_message: Optional[str] = Field(default=None, description="Error message if sync failed")