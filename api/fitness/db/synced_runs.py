...
def create_synced_run(
    ...
    with get_db_cursor() as cursor:
        ...
        """, (run_id, run_version, google_event_id, now, sync_status, error_message, now, now))
        
        result = cursor.fetchone()
        sync_id, created_at, updated_at = result
        
        logger.info(f"Created sync record for run {run_id} with Google event {google_event_id}")
