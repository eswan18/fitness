...
def test_complete_shoe_lifecycle(client):
    ...
    test_shoe_mileage = next((shoe for shoe in shoe_mileage if shoe["shoe"]["name"] == shoe_name), None)
    ...
    retired_test_shoe = next((shoe for shoe in retired_shoes if shoe["id"] == shoe_id), None)
    ...
    retired_shoe_mileage = next((shoe for shoe in all_shoe_mileage if shoe["shoe"]["name"] == shoe_name), None)
    ...
    unretired_shoe = next((shoe for shoe in active_shoes_final if shoe["id"] == shoe_id), None)
...
def test_multiple_shoes_management(client):
    ...
    # Retire road shoe
    res = client.patch(f"/shoes/{road_shoe_id}", json={
        "retired_at": "2024-11-15",
        "retirement_notes": "Worn out treads"
    })
    assert res.status_code == 200
    
    # Retire trail shoe
    res = client.patch(f"/shoes/{trail_shoe_id}", json={
        "retired_at": "2024-11-20", 
        "retirement_notes": "Sole separation"
    })
    assert res.status_code == 200
    ...
    road_shoe_retired = next((shoe for shoe in retired_shoes if shoe["name"] == "Road Shoe A"), None)
    trail_shoe_retired = next((shoe for shoe in retired_shoes if shoe["name"] == "Trail Shoe C"), None)
...
def test_shoe_error_cases(client):
    ...
    res = client.patch(f"/shoes/{fake_shoe_id}", json={
        "retired_at": "2024-01-01",
        "retirement_notes": "test"
    })
    ...
    res = client.patch(f"/shoes/{shoe_id}", json={
        "retired_at": "invalid-date-format",
        "retirement_notes": "test"
    })
...
def test_shoe_filtering_behavior(client):
    ...
    res = client.patch(f"/shoes/{retired_shoe_id}", json={
        "retired_at": "2024-01-15",
        "retirement_notes": "test retirement"
    })
    assert res.status_code == 200
...
def test_shoe_mileage_consistency(client):
    ...
    initial_shoe = next((shoe for shoe in initial_mileage if shoe["shoe"]["name"] == shoe_name), None)
    ...
    retired_shoe = next((shoe for shoe in retired_mileage if shoe["shoe"]["name"] == shoe_name), None)
    ...
    active_shoe = next((shoe for shoe in active_only_mileage if shoe["shoe"]["name"] == shoe_name), None)
    ...
    final_shoe = next((shoe for shoe in final_mileage if shoe["shoe"]["name"] == shoe_name), None)
