...
def test_retire_shoe_endpoint(client, temp_retirement_config, monkeypatch):
    ...
    monkeypatch.setattr("fitness.app.metrics.RetirementService", mock_retirement_service)
...
def test_retire_shoe_without_notes(client, temp_retirement_config, monkeypatch):
    ...
    response = client.post(
        "/metrics/shoes/Nike Air Zoom/retire",
        json={"retirement_date": "2024-12-15"}
    )
...
def test_mileage_by_shoe_with_include_retired_param(client, temp_retirement_config, monkeypatch):
...
def test_mileage_by_shoe_with_retirement_endpoint(client, temp_retirement_config, monkeypatch):
